...
func LoadCertificate(certificateBytes []byte) (certificate *x509.Certificate, err error) {
...
func LoadPrivateKey(privateKeyBytes []byte) (privateKey *rsa.PrivateKey, err error) {
	...
	if !ok {
		return nil, fmt.Errorf("%s is not rsa private key", privateKeyBytes)
	}
	return privateKey, nil
}

// LoadPublicKey 通过公钥的文本内容加载公钥
func LoadPublicKey(publicKeyBytes []byte) (publicKey *rsa.PublicKey, err error) {
	...
	if !ok {
		return nil, fmt.Errorf("%s is not rsa public key", publicKeyBytes)
	}
